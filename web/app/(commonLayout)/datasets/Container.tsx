...
const Container = () => {
  ...
  useEffect(() => {
    if (currentWorkspace.role === 'normal')
      return router.replace('/explore/apps')
  }, [currentWorkspace, router])

  return (
    <div
      ref={containerRef}
      className="scroll-container relative flex grow flex-col overflow-y-auto bg-background-body bg-default-background"
    >
      <div className="sticky top-0 z-10 flex flex-wrap justify-between gap-y-2 bg-background-body bg-default-background px-12 pb-2 pt-4">
        <TabSliderNew
          value={activeTab}
