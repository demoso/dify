...
const ModelIcon: FC<ModelIconProps> = ({
 ...
 return (
      <div className="flex items-center justify-center">
        <OpenaiBlue className={cn('h-5 w-5', className)} />
      </div>
    )
}
 ...
 return (
      <div className="flex items-center justify-center">
        <OpenaiViolet className={cn('h-5 w-5', className)} />
      </div>
    )
}

  if (provider?.icon_small) {
    let src = renderI18nObject(provider.icon_small, language)
    if (!src.startsWith('http')) {
      src = `${process.env.NEXT_PUBLIC_API_PREFIX}${src.replace(
        '/console/api',
        '',
      )}`
    }
