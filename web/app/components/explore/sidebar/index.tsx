...
const SideBar: FC<IExploreSideBarProps> = ({ controlUpdateInstalledApps }) => {
  ...
  const pinnedAppsCount = installedApps.filter(
    ({ is_pinned }) => is_pinned
  ).length;
  return (
    // border-r border-divider-burn
    <div className="w-fit shrink-0 cursor-pointer px-4 pt-6 mr-4 sm:w-[216px] bg-background-body rounded-lg">
      <div
        className={cn(
          isDiscoverySelected ? "text-text-accent" : "text-text-tertiary"
        )}
      >
        <Link
          href="/explore/apps"
          className={cn(
            isDiscoverySelected
              ...
              : "font-medium hover:bg-state-base-hover",
            "flex h-9 shadow-md items-center gap-2 rounded-lg px-3 mobile:w-fit mobile:justify-center mobile:px-2 pc:w-full pc:justify-start hover:text-text-accent"
          )}
        >
      ...
      {installedApps.length > 0 && (
        <div className="mt-10">
          <p className="break-all pl-2 text-xs font-medium uppercase text-text-tertiary mobile:px-0">
              ...
              (
                {
                  ...
                  is_pinned,
                  uninstallable,
                  app: { name, icon_type, icon, icon_url, icon_background },
                },
                index
              ) => (
                <React.Fragment key={id}>
                  ...
                  />
                  {index === pinnedAppsCount - 1 &&
                    index !== installedApps.length - 1 && <Divider />}
                </React.Fragment>
              )
            )}
          </div>
