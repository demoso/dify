...
const AppCard = ({ app, canCreate, onCreate, isExplore }: AppCardProps) => {
  ...
  return (
    <div
      className={cn(
        "group relative col-span-1 flex cursor-pointer flex-col overflow-hidden rounded-lg  bg-components-panel-on-panel-item-bg pb-2 transition-all duration-200 ease-in-out hover:shadow-lg"
      )}
    >
      ...
      {isExplore && canCreate && (
        <div
          className={cn(
            "absolute bottom-0 left-0 right-0 hidden bg-gradient-to-t from-components-panel-gradient-2 from-[60.27%] to-transparent p-4 pt-8 group-hover:flex"
          )}
        >
