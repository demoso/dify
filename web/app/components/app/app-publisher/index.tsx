...
const AppPublisher = ({
  ...
  if (process.env.NODE_ENV === 'development') {
    appURL_location
      = process.env.NEXT_PUBLIC_PUBLIC_API_PREFIX?.replace('/api', '') || ''
  }
 else {
    appURL_location = window.location.origin
  }
  // const appURL = `${appBaseURL}/${appMode}/${accessToken}`
  const appURL = `${appURL_location}/${appMode}/${accessToken}`
  ...
  const handleOpenInExplore = useCallback(async () => {
    try {
      const { installed_apps }: any
        = (await fetchInstalledAppList(appDetail?.id)) || {}
