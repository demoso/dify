...
const LogAnnotation: FC<Props> = ({ pageType }) => {
  ...
  return (
    <div className="p-4 bg-default-background h-full">
      <div className="flex h-full flex-col px-6 pt-3 bg-components-panel-bg rounded-lg">
        {appDetail.mode !== "workflow" && (
          <TabSlider
            ...
            onChange={(value) => {
              router.push(
                `/app/${appDetail.id}/${
                  value === PageType.log ? "logs" : "annotations"
                }`
              );
            }}
            options={options}
