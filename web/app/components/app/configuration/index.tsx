...
const Configuration: FC = () => {
  ...
  const latestPublishedAt = useMemo(
    () => appDetail?.model_config?.updated_at,
    [appDetail]
  );
  ...
  useEffect(() => {
    if (hasFetchedDetail && !modelModeType) {
      const mode = currModel?.model_properties.mode as
        | ModelModeType
        | undefined;
      if (mode) {
  ...
  const isShowDocumentConfig = !!currModel?.features?.includes(
    ModelFeatureEnum.document
  );
  const isAllowVideoUpload = !!currModel?.features?.includes(
    ...
    return {
      ...
      file: {
        image: {
          ...
          transfer_methods: modelConfig.file_upload?.image
            ?.transfer_methods || ["local_file", "remote_url"],
        },
        enabled: !!(
          modelConfig.file_upload?.enabled ||
          modelConfig.file_upload?.image?.enabled
        ),
        ...
        allowed_file_upload_methods: modelConfig.file_upload
      ...
      suggested: modelConfig.suggested_questions_after_answer || {
        enabled: false,
      },
  ...
  const handleFeaturesChange = useCallback(
    (flag: any) => {
      ...
      if (flag) formattingChangedDispatcher();
    },
                      ...
                      .map((tool: any) => {
                        ...
                        return {
                          ...
                          isDeleted: res.deleted_tools?.some(
                            (deletedTool: any) =>
                              deletedTool.id === tool.id &&
                              deletedTool.tool_name === tool.tool_name
                          ),
                          notAuthor:
                            toolInCollectionList?.is_team_authorization ===
                            false,
        ...
        const retrievalConfig = getMultipleRetrievalConfig(
          {
            ...
            reranking_model: modelConfig.dataset_configs.reranking_model && {
              provider:
                modelConfig.dataset_configs.reranking_model
                  .reranking_provider_name,
              model:
                modelConfig.dataset_configs.reranking_model
                  .reranking_model_name,
  ...
  return (
    ...
    >
      <FeaturesProvider features={featuresData}>
        <>
          <div className="flex h-full flex-col bg-default-background p-4">
            <div className="relative flex h-[200px] grow bg-background-body rounded-lg pt-14">
              {/* Header */}
                ...
                <div className="flex h-14 items-center justify-between px-6">
                  <div className="flex items-center">
                    ...
                    <div className="flex h-[14px] items-center space-x-1 text-xs">
                      {isAdvancedMode && (
                        <div className="system-xs-medium-uppercase ml-1 flex h-5 items-center rounded-md border border-components-button-secondary-border px-1.5 uppercase text-text-tertiary">
              ...
              <div
                className={`flex h-full w-full shrink-0 flex-col sm:w-1/2 ${
                  debugWithMultipleModel && "max-w-[560px]"
                }`}
              >
                <Config />
              </div>
