...
const PluginsPanel = () => {
  ...
  const { data: pluginList, isLoading: isPluginListLoading } =
    useInstalledPluginList();
  ...
  const pluginListWithLatestVersion = useMemo(() => {
    return (
      pluginList?.plugins.map((plugin) => ({
        ...plugin,
        latest_version:
          installedLatestVersion?.versions[plugin.plugin_id]?.version ?? "",
        latest_unique_identifier:
          installedLatestVersion?.versions[plugin.plugin_id]
            ?.unique_identifier ?? "",
      })) || []
    );
  ...
  const filteredList = useMemo(() => {
    ...
    const filteredList = pluginListWithLatestVersion.filter((plugin) => {
      return (
        (categories.length === 0 ||
          categories.includes(plugin.declaration.category)) &&
        (tags.length === 0 ||
          tags.some((tag) => plugin.declaration.tags.includes(tag))) &&
        (searchQuery === "" ||
  ...
  return (
    <div className="bg-components-panel-bg rounded-b-lg">
      <div className="flex flex-col items-start justify-center gap-3 self-stretch px-12 pb-3 pt-1">
        ...
        <FilterManagement onFilterChange={handleFilterChange} />
      </div>
      {isPluginListLoading ? (
        <Loading type="app" />
