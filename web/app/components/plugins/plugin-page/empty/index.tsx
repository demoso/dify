...
const Empty = () => {
  ...
  const text = useMemo(() => {
    if (pluginList?.plugins.length === 0) return t("plugin.list.noInstalled");
    if (
    ...
    <div className="relative z-0 w-full grow">
      ...
      <div className="absolute top-0 z-10 grid h-full w-full grid-cols-2 gap-2 overflow-hidden px-12">
        {Array.from({ length: 20 })
          ...
          .map((_, i) => (
            <div
              key={i}
              className="h-[100px] rounded-xl bg-components-card-bg"
            />
          ))}
      </div>
      {/* mask */}
      ...
      <div className="relative z-30 flex h-full items-center justify-center">
        <div className="flex flex-col items-center gap-y-3">
          <div
            className="relative -z-10 flex h-[52px] w-[52px] items-center justify-center rounded-xl
          ...
          <div className="text-sm font-normal text-text-tertiary">{text}</div>
              ...
              {[
                ...(enable_marketplace && true
                  ? [
                      {
                        icon: MagicBox,
                        text: t("plugin.list.source.marketplace"),
                        action: "marketplace",
                      },
                    ]
                  : []),
